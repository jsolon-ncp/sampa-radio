...
postfile `memhold'  bias biassd biaslci biasuci str10 rater str50 my1 str50 my2 using "data/table1_blandaltman_results_all.dta", replace
...
foreach var of local cat_vars {
    ...
    foreach rater of local readerlv {
        use "data/SAMPA_long_icc.dta", clear
		
		//drop other cohort, retain only STATT and CLHNS
		drop if cohort < 5
		
		keep if instance==`rater' & index(var, "`var'")
		...
		* Display the selected variables (optional)
		...
		graph export "./figures/table1/bland_difference_all_`my1'_`my2'.svg", replace
		graph save "./figures/table1/gph/bland_difference_all_`my1'_`my2'.gph", replace
		...
		* Display the selected variables (optional)
		...
		graph export "./figures/table1/bland_percentmean_all_`my1'_`my2'.svg", replace
...
use "data/table1_blandaltman_results_all.dta", clear
...
keep rater biaslimits var


generate cohort=9

save "data/table1_blandaltman_results_all.dta", replace
...
postfile `memhold'  bias biassd biaslci biasuci c str10 rater str50 my1 str50 my2 using "data/table1_blandaltman_results_cohort.dta", replace
...
foreach var of local cat_vars {
	...
	foreach c of local cohortlv {
		...
		foreach rater of local readerlv {
			...
			keep if instance==`rater' & cohort==`c' & index(var, "`var'")
		...
		display "`biasuci'"	
			...
			* Display the selected variables (optional)
			...
			graph export "./figures/table1/bland_difference_`c'_`my1'_`my2'.svg", replace
			graph save "./figures/table1/gph/bland_difference_`c'_`my1'_`my2'.gph", replace
			...
			* Display the selected variables (optional)
			...
			graph export "./figures/table1/bland_percentmean_`c'_`my1'_`my2'.svg", replace
...
// tidying up the dataset
use "data/table1_blandaltman_results_cohort.dta", clear
...
save "data/table1_blandaltman_results_cohort.dta", replace
...
append using "data/table1_blandaltman_results_all.dta"
...
save "data/table1_blandaltman_results_deliv.dta", replace
...
putdocx text ("Table 1.1: Intra-rater bias estimate (horizontal)")
...
putdocx save "report/table_1.1.docx", replace
