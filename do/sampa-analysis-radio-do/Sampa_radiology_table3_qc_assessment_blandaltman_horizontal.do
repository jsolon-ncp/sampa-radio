...
postfile `memhold'  bias biassd biaslci biasuci str10 rater str50 my1 str50 my2 using "data/table3_blandaltman_results_all.dta", replace
...
foreach var of local cat_vars {
	...
	drop if cohort < 5	
		...
		* Correctly reference macros for the blandaltman command
        ...
        post `memhold'  (`bias') (`biassd') (`biaslci') (`biasuci') ("`rater'") ("`my1'") ("`my2'")
		...
		* Display the selected variables (optional)
		...
		graph export "./figures/table3/bland_difference_all_`my1'_`my2'.svg", replace
		graph save "./figures/table3/gph/bland_difference_all_`my1'_`my2'.gph", replace
		...
		* Display the selected variables (optional)
		...
		graph export "./figures/table3/bland_percentmean_all_`my1'_`my2'.svg", replace
...
use "data/table3_blandaltman_results_all.dta", clear
...
save "data/table3_blandaltman_results_all.dta", replace
...
postfile `memhold'  bias biassd biaslci biasuci c str10 rater str50 my1 str50 my2 using "data/table3_blandaltman_results_cohort.dta", replace
	...
	foreach c of local cohortlv {
		...
		display "`biasuci'"	
			...
			* Display the selected variables (optional)
			...
			graph export "./figures/table3/bland_difference_`c'_`my1'_`my2'.svg", replace
			graph save "./figures/table3/gph/bland_difference_`c'_`my1'_`my2'.gph", replace
			...
			* Display the selected variables (optional)
			...
			graph export "./figures/table3/bland_percentmean_`c'_`my1'_`my2'.svg", replace
...
// tidying up the dataset
use "data/table3_blandaltman_results_cohort.dta", clear
...
keep biaslimits c var


rename c cohort

save "data/table3_blandaltman_results_cohort.dta", replace
...
append using "data/table3_blandaltman_results_all.dta"




reshape wide biaslimits, i(cohort) j(var) string



sort cohort 
order cohort  biaslimitsbody_tail biaslimitsbody_trans biaslimitscc_body biaslimitscc_head biaslimitshead_ap biaslimitshead_trans biaslimitstail_trans biaslimitsvol
...
save "data/table3_blandaltman_results_deliv.dta", replace
...
putdocx text ("Table 3.1: Inter-rater bias estimate (horizontal): Radio 2 vs Radio 3")

putdocx table out3_uni_gsem=data(cohort biaslimitsbody_tail biaslimitsbody_trans biaslimitscc_body biaslimitscc_head biaslimitshead_ap biaslimitshead_trans biaslimitstail_trans biaslimitsvol), halign(center) varnames layout(autofitc) border(all, nil) width(100%) 
...
putdocx save "report/table_3.1.docx", replace
