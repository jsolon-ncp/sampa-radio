...
cap prog drop kwstats
program kwstats, rclass
...
local tobitvars "ever_mal sex bmi_cat cohort africa child exposure hiv "

foreach var of varlist `tobitvars' {
	collect _r_b _r_ci _r_p : tobit fecal_elastase i.`var', ul(600)
}

collect layout (colname) (result[_r_b _r_ci _r_p])


collect style use ./tables/st_regress.stjson, replace


** Kwallis other enzymes

	foreach v in ul_amylp amyl_adj lipase ngml_trypsinogen {
    quietly kwallis `v', by(`col')
    collect: kwstats
}

collect _r_p : kwallis fecal_elastase ever_mal, ul(600)
* Main comparison

dtable, by(ever_mal, tests) ///
continuous(fecal_elastase ul_amylp lipase ngml_trypsinogen, ///
statistics(p50 iqr) test(kwallis)) 

* Possible covariates
local byvars "ever_mal bmi_cat sex africa child exposure hiv cohort"
foreach var of varlist `byvars' {
	dtable, by(`var', tests) ///
	continuous(fecal_elastase ul_amylp lipase ngml_trypsinogen, ///
	statistics(p50 iqr) test(kwallis))À
}
...
stat(p75 ul_amylp) ///
stat(iqr ul_amylp) ///
command(kwallis ul_amylp, by(bmi_cat)) ///


kwallis ul_amylp, by(sex)

kwallis ul_amylp, by(africa)
kwallis ul_amylp, by(child)
kwallis ul_amylp, by(exposure)
kwallis ul_amylp, by(hiv)
kwallis ul_amylp, by(cohort)


scatter ul_amylp age

* 2. Basic Model
* -------------
* Model 1: Basic model with main effects only

regress ul_amylp i.ever_mal i.bmi_cat mgl_cystatin i.africa i.child i.exposure i.hiv
regress ul_amylp i.ever_mal i.bmi_cat egfr

i.bmi_cat i.africa i.child i.exposure i.hiv 

regress ul_amylp i.ever_mal i.bmi_cat i.africa i.child i.exposure i.hiv 
	estimates store m1
