...
* GENERATE TABLE STATS 
	...
	table (var)(`scol' `col') , ///
		statistic(mean `contn') ///
		statistic(sd `contn') ///
		statistic(fvfrequency `ind' `cat') ///
		statistic(percent i.`ind'  i.`cat', across(`col')) ///
			nototals ///
...
* CREATE BASE LAYOUT FOR TABLE 1 Layout Super column - column 

	* collect layout (var) (region#diabetes#result[column1 column2])

	collect layout (var) (`scol'#`col'#result[column1 column2])
...
* format means variable column 1 and column 2
	...
	collect style cell var[`contn']#result[column2], nformat(`nfrmt_sd') /* Formats continuous variable to one decimal*/
...
1. Hide selected levels of a variable  for instance showin only Poor hlthstat or Show only females
2. Modify text of each variable : for instance "Body mass index (BMI)" to "BMI"
3. Add t-tests across group ie, local `col' as columns.
4. Export to word using collect, export if possible formatted such that columns are already resized and readable 
