...
global dropboxradio = "~/Cox working group Dropbox/TB Nutrition working group/SAMPA/Data Analysis/Radiology/"
...
* ever_mal in crf11b is not clean
	...
	keep sampa_id lowelastase verylowelastase 
	sort sampa_id
	save `1'
...
* generate us or ct
gen usct=0
replace usct = 1 if radio_exam ==1 | ct_scan ==1

dtable 1.radio_exam 1.ct_scan , by(usct) 
...
* genreate pancreas volume

gen panvol = 3.1415927 * 4/3 * (((pan_head_ap +  pan_head_trans)/2)^3)
la var panvol "Pancreas Volume US"


* Generate single pancreas structural abnoralit us and ct?
* ct
local ct "ct_pan_shape ct_pan_contour ct_pan_parenchyma ct_pan_paren_inflam ct_pan_paren_fibrosis ct_pan_paren_steatosis ct_pan_duct ct_pan_duct_stones ct_pan_duct_mass ct_pan_calci"
...
* CREATE STRUCTURAL ABNORALITIES VARIABLES
...
tab strabn_us strabn_ct, mis col row 
...
label values bmi_cat_child bmi_cat_child
 ...
 u
 dtable i.strabn_us i.strabn_ct i.strabn_all, by(usct) export(strabn.docx, as(docx) replace)

gr bar (count) strabn_us strabn_ct if strabn_us==1
se "${sampadata}/All_Crf11b_Urine_And_Feces_Results.dta", clear
 ...
 ** graph 
 ...
 baplot pan_head_trans ct_pan_head_trans
 blandaltman pan_head_trans ct_pan_head_trans, plot(percentmean)
**# TABLE 1
